...
const Register = () => {
  ...
  const validate = () => {
    ...
    if (!/^[a-zA-Z\s]+$/.test(formData.name)) newErrors.name = 'Enter a valid name.';
    if (!/^\S+@\S+\.\S+$/.test(formData.email)) newErrors.email = 'Invalid email format.';
    if (!/^[6-9]\d{9}$/.test(formData.phoneNumber)) newErrors.phoneNumber = 'Invalid phone number.';
    if (!/^(?=.*[A-Za-z])(?=.*\d)[A-Za-z\d]{8,}$/.test(formData.password))
      newErrors.password = 'Min 8 characters, at least 1 letter and 1 number.';
    if (formData.password !== formData.confirmPassword) newErrors.confirmPassword = 'Passwords do not match.';
    if (!formData.gender) newErrors.gender = 'Please select gender.';
    if (formData.role === 'TENANT' && !formData.userType) newErrors.userType = 'Please select tenant type.';
      ...
      <div className="register-container">
        <div className="role-toggle">
          ...
          <button
            ...
            onClick={() => setFormData({ ...formData, role: 'OWNER', userType: '' })}
          >
            I'm an Owner
          </button>
        </div>
        ...
        <form className="register-form" onSubmit={handleSubmit}>
          ...
          <input type="password" name="password" placeholder="Create a strong password" value={formData.password} onChange={handleChange} />
          ...
          <div className="checkbox-container">
           ...
           <label>
  I agree to StayNest’s <a href="/terms">Terms of Service</a> and <a href="/privacy">Privacy Policy</a>
</label>
